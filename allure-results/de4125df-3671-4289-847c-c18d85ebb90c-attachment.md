...
Error: strict mode violation: locator('select') resolved to 2 elements:
    1) <select class="px-3 py-2 text-sm rounded-lg border border-neutral-200 bg-neutral-0 text-neutral-700 outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-all duration-fast">…</select> aka locator('main select')
    2) <select class="text-xs px-2 py-1.5 rounded-lg border border-neutral-200 bg-neutral-0 text-neutral-700 outline-none focus:border-primary-500 cursor-pointer disabled:opacity-60">…</select> aka getByRole('combobox')
...
```ts
  ...
  3  | 
  4  | test('User can view new request', async ({ page }) => {
  ...
  13 | 
  14 |   // Проверка: в элементе select есть опция "Новая"
  15 |   const statusSelect = page.locator('select');
> 16 |   await expect(statusSelect).toBeVisible();
     |                              ^ Error: expect(locator).toBeVisible() failed
  17 |   await expect(statusSelect).toContainText('Новая');
  18 | });
```